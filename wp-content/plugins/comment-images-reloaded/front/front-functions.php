...
class CIR_Front{
	...
	function add_image_upload_form( $post_id ) {
		...
		if( 'disable' != $all_posts_state  && $current_post_state == 'enable' ){
			...
			$html = '<div id="comment-image-reloaded-wrapper" class="fileform">';
			$html .= '<p id="comment-image-reloaded-error"></p>';
			$html .= "<p class='comment-image-reloaded'><div id='fileformlabel'></div><div class='selectbutton'>Загрузить фото</div><input type='file' name=$input_name id='comment_image_reloaded' multiple='multiple' onchange='getName(this.value);' /></p>";
			$html .= $brand_img;
			$html .= '</div>';
...
?>
			...
			<script>
				jQuery(document).ready(function () {
					var upload = jQuery('#comment-image-reloaded-wrapper').html();
					upload = '<div id="comment-image-reloaded-wrapper" class="fileform">' + upload + '</div>';
					jQuery('.form-submit').before(upload);
					jQuery('.form-submit').next().remove();
				});
			...
			foreach( $comments as $comment ) {
				...
				if( !empty($metadata_ids[$comment->comment_ID]) ) {
					...
					if (isset(self::$options['image_zoom']) && 'enable' == self::$options['image_zoom']) {
						...
						foreach($img_url_out as $id => $img_out) {
							$src = $img_out;
							if (preg_match("/(src)=\".+(jpg|png)\"/", $src, $match)) {
								$link_img =  substr($match[0], 4 );
							}
							
							$comment->comment_content .= '<a class="image-popup" href=';
							$comment->comment_content .= $link_img;
